...
class Retriever:
    ...
    def save(self, index_path="storage/index.faiss", chunks_path="storage/chunks.pkl"):
        ...
        return f"Данные успешно сохранены в {index_path} и {chunks_path}"

    def load(self, index_path="storage/index.faiss", chunks_path="storage/chunks.pkl"):
        assert os.path.exists(chunks_path), f"Файл {chunks_path} не найден"
        assert os.path.exists(index_path), f"Файл {index_path} не найден"
